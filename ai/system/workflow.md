...
When the repository already exists and is being adopted into the template after delivery has started, prepend that flow with:
...
7. When strict convergence enforcement is enabled, the pipeline audits architecture and core source docs after specification work and blocks downstream stages if placeholders or template references remain.
8. Orchestrator ensures exactly one task is `ready`.
9. Pre-stage security validation runs against the routed inputs and the stage guard before every agent step.
10. UX/UI Designer refines frontend-facing slices so UI states, accessibility, and responsive behavior are explicit before implementation.
11. Builder implements the ready slice.
12. Reviewer checks architecture and contract compliance.
13. Tester runs the relevant verification.
14. Blocking quality gates run for the active slice, including lint, typecheck, unit, e2e, coverage, and frontend evidence when configured.
15. Frontend Auditor reviews user-visible quality, accessibility, responsiveness, and frontend evidence for frontend-facing slices.
16. Post-stage security validation runs against the changed file set and the stage guard.
17. Security audits trust boundaries and isolation.
18. Orchestrator records the result and commits the slice when appropriate.
...
- when `AI_ENFORCE_SPEC_CONVERGENCE=1`, block downstream execution unless core architecture and source docs are placeholder-free and no template references remain in registry/index artifacts
