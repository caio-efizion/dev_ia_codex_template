...
specs_summary() {
  local spec_total
  spec_total=$(find "${SOURCE_DIR}/specs" -maxdepth 1 -type f -name '*.md' ! -name 'README.md' | wc -l | tr -d ' ')
...
EOF
    ...
    find "${SOURCE_DIR}/specs" -maxdepth 1 -type f -name '*.md' ! -name 'README.md' | sort | while read -r spec_file; do
